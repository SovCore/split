...
export default function App() {
  const [topState, dispatch] = useReducer(
    topLevelReducer,
    TopLevelState.INITIALIZING,
  );
    ...
    <DemoGateWrapper>
      <div className="bg-background text-on-background min-h-screen flex flex-col font-body-md text-body-md">
        ...
        <div className="flex flex-1 relative w-full max-w-container-max-width mx-auto">
                  ...
                  <li>
                    ...
                    >
                      <span
                        ...
                        style={
                          topState === TopLevelState.SPLIT_FLOW
                  ...
                  <li>
                    ...
                    >
                      <span
                        className="material-symbols-outlined"
                        style={
                          topState === TopLevelState.RECOVER_FLOW
                            ? { fontVariationSettings: "'FILL' 1" }
                            : {}
                        }
                      >
          ...
          {/* Main Content Area */}
          <main
            className={`flex-1 ${topState !== TopLevelState.INITIALIZING && topState !== TopLevelState.UNSUPPORTED_ENVIRONMENT ? "md:ml-64 print:ml-0" : ""} p-margin pt-12 pb-24 print:p-0 print:pt-0 min-h-[calc(100vh-64px-48px)]`}
          >
            {topState === TopLevelState.INITIALIZING && (
            ...
            {topState === TopLevelState.UNSUPPORTED_ENVIRONMENT && (
              <div className="bg-error-container text-on-error-container p-6 rounded-lg max-w-2xl mx-auto">
                ...
                <p>
                  The browser does not support WebAssembly or is missing
            ...
            {topState === TopLevelState.DASHBOARD && (
              <div className="max-w-3xl mx-auto text-center">
                ...
                <p className="text-on-surface-variant text-body-md mb-8 leading-relaxed">
                  SovCore Split is a free, friction-free utility for
                  distributing sensitive cryptographic payloads. Shard core root
                  credentials, master system access keys, or recovery phrases
                  using isolated polynomial distribution math.
                  <br />
                  <br />
                  100% local WASM execution operating entirely within volatile,
                  isolated memory enclaves.
                  <br />
                  <br />
                  <strong>Zero</strong> background caching,{" "}
                  <strong>Zero</strong> server persistence, and{" "}
                  <strong>Zero</strong> outbound network telemetry.
                </p>
                <h2 className="font-headline-lg text-headline-lg mt-12 mb-4 text-on-surface">
                  Select an Operation
                </h2>
            ...
            {topState === TopLevelState.SPLIT_FLOW && (
              <SplitFlow onComplete={() => dispatch({ type: "GO_HOME" })} />
            )}
            {topState === TopLevelState.RECOVER_FLOW && (
              <AssembleFlow onComplete={() => dispatch({ type: "GO_HOME" })} />
            )}
          </main>
        </div>
        ...
        <footer className="bg-surface-container dark:bg-surface-container text-on-surface-variant font-code-md text-code-md font-bold text-primary w-full h-12 border-t border-outline-variant/20 flat no shadows fixed bottom-0 flex items-center justify-between px-margin py-2 z-50 print:hidden">
          <div className="text-on-surface-variant/70">
            © 2026 SovCore. All rights reserved.
          </div>
          <div className="flex space-x-6">
            <a
              className="text-on-surface-variant/40 cursor-not-allowed flex items-center gap-1"
              title="Coming Soon"
            >
              Security Policy
              <span className="text-[10px] bg-surface-container-high px-1 rounded">
                Soon
              </span>
            </a>
            <a
