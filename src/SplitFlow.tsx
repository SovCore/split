  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="hidden print:block print:p-8">
        <h1 className="text-2xl font-bold mb-2">SovCore Split - Paper Backup</h1>
        <p className="text-sm mb-4">Threshold (k): {k}, Total Shards (n): {n}</p>
        <hr className="mb-6 border-black" />
        <div className="space-y-6">
          {shares.map((share, i) => (
            <div key={i} className="p-4 border border-gray-400 font-mono">
              <h3 className="font-bold mb-2">SHARD #{i + 1}</h3>
              <p className="break-all text-[12px]">{share}</p>
            </div>
          ))}
        </div>
      </div>
